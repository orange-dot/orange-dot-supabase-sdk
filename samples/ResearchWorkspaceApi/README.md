...
Broader ASP.NET Core sample for the server-side `AddSupabaseServer(...)` flow.

This sample shows the SDK across a wider Supabase surface:
...
- Server-side SDK path
  The ASP.NET app uses `AddSupabaseServer(...)` and `ISupabaseStatelessClientFactory` for delegated PostgREST, Storage, Functions, and Realtime operations.
- Browser cockpit
  The embedded UI in `wwwroot/` is static HTML, CSS, and JavaScript. It talks to the sample API with `fetch` and stores the returned access token in browser storage.
- UI auth helper
  The sample-scoped `/ui/auth/signup` and `/ui/auth/login` endpoints call the local Supabase Auth HTTP API with the publishable key to create or exchange a browser session. They do not add a separate browser SDK wrapper or a server-side cookie session.
...
Protected endpoints return JSON error payloads in this shape:
...
Protected endpoints verify that bearer token against Supabase Auth before they trust caller identity for `/me` or watcher ownership decisions.
Everything after that token exchange goes through the sample API, whose server-side operations are backed by the SDK.
